...
3. Now we can use the t2d command. This will be accomplish by using the next
   command line:
    ...
    ```bash
    $ t2d git@git.vauxoo.com:vauxoo/marin.git 17.0
    ```
    ...
    ```bash
    ['${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_marin.git/17.0/3.10/env_1_job_1',
     '${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_marin.git/17.0/3.10/env_2_job_1']
    ```
...
4. The next step is to run the last mentioned scripts, we highly recommend to
   add the next extra arguments.
    ...
    ```bash
    ${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_marin.git/17.0/3.10/env_2_job_1/10-build.sh --no-cache
    ${HOME}/t2d/script/git_git.vauxoo.com_vauxoo_marin.git/17.0/3.10/env_2_job_1/20-run.sh --entrypoint=bash
    ```

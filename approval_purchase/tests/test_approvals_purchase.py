...
class TestApprovalsPurchase(TestApprovalsCommon):
    ...
    def test_02_check_constrains(self):
        ...
        self.assertEqual(request_purchase.count_purchase_order, 0)
        ...
        request_purchase.action_create_purchase_orders()
        self.assertEqual(request_purchase.count_purchase_order, 1)
    ...
    def test_purchase_04_create_multiple_purchase(self):
        ...
        self.assertEqual(
            request_purchase.count_purchase_order,
            2,
    ...
    def test_uom_01_create_purchase(self):
        ...
        self.product_earphone.seller_ids.product_uom_id = self.env.ref(
            "uom.product_uom_day"
        )
        ...
        self.assertEqual(
            purchase_order.order_line[0].product_uom_id.id,
    ...
    def test_uom_02_create_purchase(self):
        ...
        request_product_line = request_purchase.product_line_ids[0]
        purchase_order = self.get_purchase_order(request_purchase, 0)
        self.assertEqual(request_product_line.product_uom_id.id, uom_day.id)
        self.assertEqual(
    ...
    def test_product_line_seller(self):
        ...
        approval = self.env["approval.request"].create(
            {
                ...
                "product_line_ids": [
                    Command.create(
                        {"product_id": product_with_vendor.id, "quantity": 15}
                    ),
        ...
        self.assertEqual(
            approval.product_line_ids[0].seller_id.partner_id, self.partner_seller_1
        )
    ...
    def test_logging_purchase_order_state_to_approval_request_chatter(self):
        ...
        purchase_orders_data = approval_request._get_order_data_from_product_lines(
            approval_request.product_line_ids
        )
        purchase_orders_creation_log_message = (
            approval_request._prepare_po_log_message("created", purchase_orders_data)
        )
        ...
        actual_logged_message = tools.html2plaintext(
            approval_request_chatter_message.body
        )
        ...
        earphone_purchase_order_approval_log_message = (
            earphone_purchase_order._prepare_state_change_msg(
                "draft", "purchase", earphone_product_line
            )
        )
        approval_request_chatter_message = approval_request.message_ids[0]
        expected_message = tools.html2plaintext(
            earphone_purchase_order_approval_log_message
        )
        actual_logged_message = tools.html2plaintext(
            approval_request_chatter_message.body
        ...
        computer_purchase_order_cancelation_log_msg = (
            computer_purchase_order._prepare_state_change_msg(
                "draft", "cancel", computer_product_line
            )
        )
        ...
        computer_purchase_order_removal_log_message = (
            approval_request._prepare_po_log_message(
                "removed", computer_purchase_order_data
            )
        )
        # The earphone purchase order isn't in draft state, So it cannot be removed.
        earphone_purchase_order_require_manual_action_log_message = (
            approval_request._prepare_po_log_message(
                "require_manual_action", earphone_purchase_order_data
            )
        )
        purchase_orders_state_change_log_message = (
            computer_purchase_order_removal_log_message
            + earphone_purchase_order_require_manual_action_log_message
        )
        expected_message = tools.html2plaintext(

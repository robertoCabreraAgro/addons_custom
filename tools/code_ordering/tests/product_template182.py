...
class ProductTemplate(models.Model):
    ...
    product_tooltip = fields.Char(
        compute="_compute_product_tooltip",
    )
    ...
    description_sale = fields.Text(
        ...
        "This description will be copied to every Sales Order, "
    ...
    count_product_variant = fields.Integer(
        ...
        compute="_compute_count_product_variant",
    )
    ...
    count_pricelist_item = fields.Integer(
        string="Number of price rules",
        compute="_compute_count_pricelist_item",
    )
    ...
    count_product_document = fields.Integer(
        string="Documents Count",
        compute="_compute_count_product_document",
    )
    ...
    def create(self, vals_list):
        """Store the initial standard price in order to be able to retrieve
    ...
    def write(self, vals):
        ...
        if (
            self._context.get("create_product_product", True)
    ...
    def copy_data(self, default=None):
        ...
        if "name" not in default:
            for template, vals in zip(self, vals_list):
                vals["name"] = _(f"{template.name} (copy)")
        return vals_list
    ...
    def _compute_count_product_document(self):
        for template in self:
            template.count_product_document = template.env[
                "product.document"
    ...
    def _compute_service_tracking(self):
        self.filtered(lambda template: template.type != "service").service_tracking = (
            "no"
        )
    ...
    def _compute_valid_product_template_attribute_line_ids(self):
        ...
        for template in self:
            template.valid_product_template_attribute_line_ids = (
                template.attribute_line_ids.filtered(lambda ptal: ptal.value_ids)
            )
    ...
    def _compute_has_configurable_attributes(self):
        ...
        for template in self:
            template.has_configurable_attributes = (
                template.has_dynamic_attributes()
                or any(ptal._is_configurable() for ptal in template.attribute_line_ids)
            )
    ...
    def _compute_product_variant_id(self):
        for template in self:
            template.product_variant_id = template.product_variant_ids[:1].id
    ...
    def _compute_count_product_variant(self):
        for template in self:
            template.count_product_variant = len(template.product_variant_ids)
        ...
        if self.env["product.template"].search_count(domain, limit=1):
            return {
                "warning": {
                    "title": _("Note:"),
                    "message": _(
                        f'The Internal Reference "{self.default_code}" already exists.'
                    ),
                }
            }
    ...
    def _onchange_uom_id(self):
        if (
            ...
            or not self.with_context(
                active_test=False
            ).product_variant_ids._check_uom_used()
        ):
            return

        message = _(
            "Changing the unit of measure for your product will apply a conversion "
            "1 %(old_uom_name)s = 1 %(new_uom_name)s.\n"
            "All existing records (Sales orders, Purchase orders, etc.) using this product "
            "will be updated by replacing the unit name.",
    ...
    def action_open_pricelist_rules(self):
        self.ensure_one()
        domain = [
            "|",
        ...
        return {
            ...
            "context": {
                ...
                "product_without_variants": self.count_product_variant == 1,
    ...
    def _create_product_variant(self, combination, log_warning=False):
        ...
        if not self.has_dynamic_attributes():
            if log_warning:
                _logger.warning(
                    f"The user #{self.env.user.id} tried to create a variant for the non-dynamic product {self.id}."
                )
            return Product
        ...
        if not self._is_combination_possible(combination):
            if log_warning:
                _logger.warning(
                    f"The user #{self.env.user.id} tried to create an invalid variant for the product {self.id}."
                )
            return Product
    ...
    def _create_variant_ids(self):
        ...
        if variants_to_unlink:
            ...
            if self.exists() != self:
                raise UserError(
                    _(
                        "This configuration of product attributes, values, and exclusions would lead to no possible variant. "
    ...
    def _get_own_attribute_exclusions(self, combination_ids=None):
        ...
        :param list combination: The combination of the product, as a
            list of `product.template.attribute.value` ids.
        Dictionnary, each product template attribute value is a key,
        and for each of them the value is an array with the other ptav
        that they exclude (empty if no exclusion)."""
        self.ensure_one()
    ...
    def get_single_product_variant(self):
        ...
        self.ensure_one()
        if self.count_product_variant == 1 and not self.has_configurable_attributes:
            return {

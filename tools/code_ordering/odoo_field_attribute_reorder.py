...
class FieldAttributeReorderer:
        ...
        class FieldVisitor(ast.NodeVisitor):
                ...
                if isinstance(node.value, ast.Call):
                    if isinstance(node.value.func, ast.Attribute):
                        if hasattr(node.value.func, 'attr'):
                            ...
                            if field_type in self.field_attribute_order or \
                               field_type in {'Char', 'Text', 'Integer', 'Float', 'Boolean',
                                            'Date', 'Datetime', 'Binary', 'Selection',
                                            'Many2one', 'One2many', 'Many2many',
                                            'Reference', 'Image', 'Json', 'Properties',
                                            'Html', 'Monetary'}:
            ...
            def _extract_attributes(self, call_node):
                ...
                for keyword in call_node.keywords:
                    if keyword.arg:
                        attrs[keyword.arg] = ast.unparse(keyword.value) if hasattr(ast, 'unparse') else str(keyword.value)
                ...
                if call_node.args:
                    ...
                    if len(call_node.args) > 0:
                        attrs['_positional_0'] = ast.unparse(call_node.args[0]) if hasattr(ast, 'unparse') else str(call_node.args[0])
    ...
    def _extract_field_definition(self, lines: List[str], start_line: int) -> Optional[str]:
    ...
    def _reorder_field_attributes(self, field_code: str, field_type: str) -> str:
        ...
        match = re.match(r'^(\s*)(\w+)\s*=\s*fields\.(\w+)\((.*)\)(.*)$',
                        field_code, re.DOTALL)
    ...
    def _parse_attributes(self, attributes_str: str) -> List[Tuple[str, str, bool]]:
    ...
    def _order_attributes(self, attributes: List[Tuple[str, str, bool]],
                         order: List[str]) -> List[Tuple[str, str, bool]]:
    ...
    def _format_multiline_field(self, indent: str, field_name: str,
                               field_class: str, attributes: List[Tuple[str, str, bool]],
                               trailing: str) -> str:
    ...
    def _format_single_line_field(self, indent: str, field_name: str,
                                 field_class: str, attributes: List[Tuple[str, str, bool]],
                                 trailing: str) -> str:
    ...
    def _apply_modifications(self, file_path: Path, original_content: str,
    ...
    def process_directory(self, directory: Path, recursive: bool = False) -> List[FieldAttributeInfo]:
    ...
    def generate_report(self, modifications: List[FieldAttributeInfo]) -> str:
        ...
        for file_path, file_mods in sorted(by_file.items()):
            ...
            for mod in file_mods:
                report_lines.append(f"  Line {mod.line_number}: {mod.field_name} ({mod.field_type})")
...
def main():
    ...
    # Create reorderer
    reorderer = FieldAttributeReorderer(

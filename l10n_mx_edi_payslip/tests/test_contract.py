...
class TestHrContract(L10nMxEdiPayslipTransactionCase):
    def test_001_gross_salary(self):
        ...
        self.env["hr.contract.gross.salary"].with_context(
            active_ids=contract.ids
        ).create({"net_salary": 50000}).set_wage()
    ...
    def test_005_work_entries(self):
        ...
        payroll.contract_id._generate_work_entries(
            payroll.date_from, payroll.date_to + timedelta(days=3)
        )
        self.assertTrue(
        ...
        payroll.contract_id.sudo().resource_calendar_id = self.env.ref(
            "resource.resource_calendar_std_35h"
        )
        self.assertTrue(
            self.env["hr.work.entry"].search(
                [("employee_id", "=", payroll.employee_id.id), ("active", "=", False)]
            ),
    ...
    def test_006_sdi_variable(self):
        ...
        payroll = self.create_payroll()
        payroll = self.create_payroll(
            payroll.date_from - relativedelta(months=2),
            payroll.date_to - relativedelta(months=2),
        )
        payroll.contract_id.date_start = payroll.contract_id.date_start - relativedelta(
            months=2
        )
        ...
        self.assertTrue(
            payroll.contract_id.l10n_mx_edi_sdi_variable, "SDI variable not assigned."
        )
    ...
    def test_007_imss_holidays_assignment(self):
        ...
        self.assertFalse(
            contract.l10n_mx_edi_imss_date, "An IMSS date has been assigned."
        )
        date_start = contract.date_start
        contract.l10n_mx_edi_imss_date = "%s-%s-%s" % (
            date_start.year - 1,
            date_start.month,
            date_start.day,
        )

...
class L10nMxEdiPayslipTransactionCase(AccountTestInvoicingCommon):
    ...
    def with_mocked_mx_payslip_pac_sucess(self):
        def fake_l10n_mx_edi_finkok_sign(record, _credentials):
            ...
            nomina_nodes = tree.xpath("//cfdi:Complemento/nomina12:Nomina", namespaces=namespaces)
            if nomina_nodes:
        ...
        with patch.object(type(self.env["hr.payslip"]), "_l10n_mx_edi_finkok_sign", fake_l10n_mx_edi_finkok_sign):
            yield
    ...
    def with_mocked_mx_payslip_pac_retry(self):
        ...
        with patch.object(type(self.env["hr.payslip"]), "_l10n_mx_edi_retry", fake_l10n_mx_edi_retry):
            yield
    ...
    def with_mocked_mx_payslip_sat_status_sucess(self):
        ...
        with patch.object(
            type(self.env["hr.payslip"]), "l10n_mx_edi_update_sat_status", fake_l10n_mx_edi_update_sat_status
        ):
            yield
    ...
    def prepare_second_employee(self, fiscal_position=False):
        ...
        account601_84 = account.copy(
            {"company_ids": [Command.link(self.company.id)], "code": "%s.%s" % (account.code, second_employee.id)}
        )
        if fiscal_position:

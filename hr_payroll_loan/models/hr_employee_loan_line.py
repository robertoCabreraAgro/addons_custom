...
class HrEmployeeLoanLine(models.Model):
    ...
    def _compute_cumulative_and_remaining_amount(self):
        for record in self:
            lines = record.loan_id.loan_line_ids.sorted(
                key=lambda line: line.sequence
            ).filtered(lambda line: line.sequence < record.sequence)
            record.cumulative_amount = record.amount + (
                lines[-1].cumulative_amount if lines else 0
            )
            if record.loan_id._is_timeless():
                record.remaining_amount = 0
                continue
            record.remaining_amount = (
                record.loan_id.total_amount
                if not lines
                else (lines[-1].remaining_amount)
            ) - record.amount
    ...
    def _unlink_except_close_or_draft(self):
        if self.filtered(
            ...
            and line.loan_id.state not in ["close", "draft"]
        ):

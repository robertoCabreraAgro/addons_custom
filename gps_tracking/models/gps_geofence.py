...
class GpsGeofence(models.Model):
    ...
    def _check_spatial_containment(self, new_geometry, container_geofence):
        ...
        _logger.info(
            f"🔍 SPATIAL CONTAINMENT CHECK - Container: {container_geofence.name} (ID: {container_geofence.id})"
        )
        ...
        if SHAPELY_AVAILABLE:
            ...
            try:
                comparison_result = self._compare_geometries(
                    container_geofence, new_geometry, new_srid=4326
                )
                ...
                if comparison_result:
                    ...
                    if not spatial_result and comparison_result.get(
                        "intersects", False
                    ):
        ...
        try:
            result = self._check_containment_postgis_improved(
    ...
    def _find_container_geofence(self, geometry_geojson, area_type):
        ...
        for geofence in bounds_candidates:
            if self._check_spatial_containment(new_geom, geofence):
                ...
                _logger.info(
                    f"✅ Spatial containment confirmed: {geofence.name} ({geofence.area_type})"
                )

        _logger.info(
            f"📊 Spatial filtering: {len(bounds_candidates)} → {len(spatial_containers)} containers"
        )

...
class GpsGeofence(models.Model):
    ...
    surface = fields.Float(string="Surface (ha)", help="Automatically calculated area in hectares", readonly=True, digits=(10, 4))
        ...
        try:
            ...
            if SHAPELY_AVAILABLE:
                try:
                    ...
                    if surface_ha and surface_ha > 0:
                        _logger.info(f"✅ Surface calculated using WKT: {surface_ha:.4f} hectares")
                        return surface_ha
            ...
            if matches:
                ...
                if 19.0 <= avg_lat <= 20.0 and -100.0 <= avg_lng <= -99.0:
                    _logger.info("Coordinates appear to be in lat/lng, using geography calculation")
            ...
            if result and result[0]:
                ...
                _logger.info(f"✅ Surface calculated using PostGIS: {surface_ha:.4f} hectares ({surface_m2:.2f} m²)")
                return surface_ha
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if vals.get('geometry'):
                _logger.info(f"📍 CREATE - Incoming geometry type: {type(vals['geometry'])}")
                _logger.info(f"📍 CREATE - Geometry preview: {str(vals['geometry'])[:200]}")
    ...
    def _project_to_srid(self, geom_json, from_srid, to_srid):
        ...
        try:
            ...
            _logger.warning(f"Error transforming geometry from {from_srid} to {to_srid}: {e}")
            return None
    ...
    def _compare_geometries(self, container_geofence, new_geojson, new_srid=4326):
        ...
        try:
            ...
            container_bounds = container_geom.bounds  # (min_lng, min_lat, max_lng, max_lat)
            new_bounds = new_geom.bounds
            
            # Step 4: Bounds check (lng=x, lat=y)
            bounds_check = (
                container_bounds[0] <= new_bounds[0] and  # min_lng
                container_bounds[2] >= new_bounds[2] and  # max_lng  
                container_bounds[1] <= new_bounds[1] and  # min_lat
                container_bounds[3] >= new_bounds[3]      # max_lat
            )
    ...
    def _check_spatial_containment(self, new_geometry, container_geofence):
        ...
        _logger.error(f"❌ ALL METHODS FAILED - Container: {container_geofence.name} (ID: {container_geofence.id})")
        _logger.error(f"New geometry type: {new_geometry.get('type', 'Unknown')}")
        if 'coordinates' in new_geometry:
            coords_preview = new_geometry['coordinates'][0][:3] if new_geometry['coordinates'] else []
    ...
    def _find_partner_in_hierarchy(self, container):
        ...
        while current:
            ...
            if current.partner_id:
                _logger.info(f"📋 Partner found in hierarchy: {current.partner_id.name} at {current.name} ({current.area_type})")
        ...
        _logger.info(f"📋 No partner found in hierarchy path: {' -> '.join(hierarchy_path)}")
        return False
    ...
    def _check_bounds_containment(self, new_bounds, container_bounds):
        ...
        contains = (
            container_bounds['min_lat'] <= new_bounds['min_lat'] and
            container_bounds['max_lat'] >= new_bounds['max_lat'] and
            container_bounds['min_lng'] <= new_bounds['min_lng'] and
            container_bounds['max_lng'] >= new_bounds['max_lng']
        )
        
        _logger.info(f"📍 BOUNDS COMPARISON:")
        _logger.info(f"   Container Lat: [{container_bounds['min_lat']:.8f}, {container_bounds['max_lat']:.8f}]")
        _logger.info(f"   New Poly  Lat: [{new_bounds['min_lat']:.8f}, {new_bounds['max_lat']:.8f}]")
        _logger.info(f"   Container Lng: [{container_bounds['min_lng']:.8f}, {container_bounds['max_lng']:.8f}]")
        _logger.info(f"   New Poly  Lng: [{new_bounds['min_lng']:.8f}, {new_bounds['max_lng']:.8f}]")
    ...
    def _find_container_geofence(self, geometry_geojson, area_type):
        ...
        try:
            new_geom = json.loads(geometry_geojson) if isinstance(geometry_geojson, str) else geometry_geojson
            _logger.info(f"📍 Parsed geometry type: {new_geom.get('type', 'Unknown')}")
            
            if new_geom.get('coordinates') and len(new_geom['coordinates']) > 0:
                coords = new_geom['coordinates'][0] if new_geom['type'] == 'Polygon' else new_geom['coordinates']
                _logger.info(f"📍 Coordinates count: {len(coords) if coords else 0}")
                if coords and len(coords) > 0:
                    lats = [c[1] for c in coords]
                    lngs = [c[0] for c in coords]
                    _logger.info(f"📍 Latitude range: [{min(lats):.8f}, {max(lats):.8f}]")
                    _logger.info(f"📍 Longitude range: [{min(lngs):.8f}, {max(lngs):.8f}]")
                    
                    # DETAILED POLYGON COORDINATES LOG
                    _logger.info("📍 BACKEND - NEW POLYGON COORDINATES (ALL POINTS):")
                    for i, coord in enumerate(coords):
                        _logger.info(f"    Point {i+1}: [{coord[0]:.8f}, {coord[1]:.8f}]")
                    ...
                    area = abs(area) / 2
                    _logger.info(f"📍 Backend - Rough polygon area: {area:.8f} square degrees")
        ...
        _logger.info(f"Found {len(potential_containers)} potential containers: {[f'{g.name}({g.area_type})' for g in potential_containers]}")
                ...
                if geom_result:
                    ...
                    try:
                        ...
                        if matches:
                            ...
                            _logger.info(f"     Lat range: [{min(container_lats):.8f}, {max(container_lats):.8f}]")
                            _logger.info(f"     Lng range: [{min(container_lngs):.8f}, {max(container_lngs):.8f}]")
                            
                            # Check if coordinates are reasonable for Mexico area
                            is_reasonable = (
                                19.0 <= avg_lat <= 20.0 and  # Toluca area latitude
                                -101.0 <= avg_lng <= -99.0    # Toluca area longitude (expanded range)
                            )
                            
                            if is_reasonable:
                                _logger.info(f"   ✅ WKT coordinates look correct for Toluca area")
                                ...
                                if self._check_bounds_containment(new_bounds, container_bounds):
                                    bounds_candidates.append(geofence)
                                    _logger.info(f"✅ {geofence.name} passes bounds check")
                                else:
                                    _logger.info(f"❌ {geofence.name} fails bounds check")
                            else:
                                _logger.warning(f"   ❌ WKT coordinates look incorrect for {geofence.name}")
                                _logger.warning(f"   Expected: Lat ~19.25, Lng ~-99.75 for Toluca")
                                
                        else:
                            _logger.warning(f"No coordinate matches found in WKT for {geofence.name}")
        ...
        for container in valid_containers:
            ...
            _logger.info(f"📋 Evaluating container: {container.name} ({container.area_type})")
            _logger.info(f"   - Sequence: {container.sequence}, Surface: {container_surface}")
            _logger.info(f"   - Has partner_id: {bool(container.partner_id)} - {container.partner_id.name if container.partner_id else 'None'}")
        ...
        if immediate_container:
            ...
            _logger.info(f"✅ Selected container: {immediate_container.name} ({immediate_container.area_type})")
            _logger.info(f"✅ Container partner_id: {immediate_container.partner_id.id if immediate_container.partner_id else 'None'}")
    ...
    def detect_and_update_container(self):
        ...
        _logger.info(f"🔄 POST-CREATE CONTAINER DETECTION for {self.name} (ID: {self.id})")
        ...
        try:
            ...
            container_info = self._find_container_geofence(geometry_geojson, self.area_type)
            ...
            if container_info.get('partner_id') and not self.partner_id:
                ...
                _logger.info(f"📋 Will inherit partner_id: {container_info['partner_id']}")
            else:
                _logger.info(f"📋 Partner inheritance skipped - container partner: {container_info.get('partner_id')}, current partner: {self.partner_id}")
            ...
            return {
                'success': False,
                'message': f'Error detecting container: {str(e)}'
            }
    ...
    def calculate_geometry_surface(self, geometry_geojson):
        ...
        try:
            ...
            if SHAPELY_AVAILABLE:
                try:
                    geometry_dict = json.loads(geometry_geojson) if isinstance(geometry_geojson, str) else geometry_geojson
                    
                    # Create Shapely geometry in EPSG:4326 (assuming GeoJSON is in lat/lng)
                    geom_4326 = shape(geometry_dict)
                    
                    # Transform to Web Mercator (EPSG:3857) for accurate area calculation
                    geom_3857 = self._project_to_srid(geometry_dict, from_srid=4326, to_srid=3857)
            ...
            if result and result[0]:
                ...
                _logger.info(f"📏 PostGIS surface calculation: {surface_ha:.4f} hectares")
                return surface_ha

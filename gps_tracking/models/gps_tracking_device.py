...
class GpsTrackingDevice(models.Model):
    ...
    #   related='last_point_id.the_point', store=True,
    # )
    # history_route = fields.GeoLine(
    #   string='History Route',
    #   compute='_compute_history_route', store=True,
    #   srid=3857,
    # )
    ...
    #        last_point = device.tracking_points.sorted(key=lambda p: p.timestamp, reverse=True)[:1]
    #        device.last_point_id = last_point.id if last_point else False
    #
    #        # Log para depurar el valor del último punto
    #        if last_point:
    #            _logger.info(f"Dispositivo {device.id}: Último punto actualizado a {last_point.id} con timestamp {last_point.timestamp}")
    #        else:
    #            _logger.warning(f"Dispositivo {device.id}: No se encontraron puntos para actualizar el último punto.")

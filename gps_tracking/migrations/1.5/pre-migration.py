...
def _check_product_asset_module(cr, env):
    ...
    if not result or result[0] != "installed":
        raise Exception(
            "product_asset module must be installed before migrating gps_tracking"
        )
...
def _log_current_state(cr, env):
    ...
    _logger.info(
        f"Current state: {device_count} devices, {point_count} points with vehicle references"
    )
...
def _check_data_integrity(cr, env):
    ...
    if orphaned_devices:
        _logger.warning(
            f"Found {len(orphaned_devices)} devices pointing to non-existent vehicles"
        )
    ...
    if unmapped_vehicles:
        _logger.warning(
            f"Found {len(unmapped_vehicles)} fleet vehicles without corresponding product assets"
        )
...
def _cleanup_analytic_line_vehicle_refs(cr):
    ...
    if invalid_count > 0:
        _logger.info(
            f"Found {invalid_count} account_analytic_line records with invalid vehicle_id references"
        )
        ...
        _logger.info(
            f"Cleaned {cleaned_count} invalid vehicle_id references (set to NULL)"
        )

...
class PlanningSlotCancel(models.TransientModel):
    ...
    cancel_details = fields.Text(tring="Additional Details")
    ...
    def _get_active_slots(self):
        ...
        if active_model == "planning.slot" and active_ids:
            if self.env["planning.slot"].browse(active_ids).filtered(lambda s: s.state in ["published", "cancelled"]):
                raise ValidationError(_("You cannot cancel assignments in published or canceled status."))
    ...
    def _send_cancellation_notification(self, slot):
        ...
        message_body = f"Hello {slot.employee_id.name},\n" \
    ...
    def action_cancel(self):
        ...
        active_ids = self.env.context.get("active_ids")
        if active_ids:
            planning_slot_ids = self.env["planning.slot"].browse(active_ids).filtered(
                lambda pl: pl.state not in ["published", "cancelled"])

    ...
    def _unlink_except_contains_maintenance_requests(self):
        for category in self:
            if category.equipment_ids or category.maintenance_ids:
                raise UserError(
                    _(
                        "You cannot delete an equipment category containing equipment or maintenance requests."
                    )
                )
    ...
    def _compute_maintenance_request(self):
        for record in self:
            ...
            record.mtbf = (
                ...
                and (record.latest_failure_date - record.effective_date).days
                / len(maintenance_requests)
                or 0
            )
        ...
        for record in self:
            if record.repeat_interval < 1:
                raise ValidationError(
                    self.env._("The repeat interval cannot be less than 1.")
                )
        ...
        if (
            ...
            for request in self:
                ...
                schedule_date += relativedelta(
                    **{f"{request.repeat_unit}s": request.repeat_interval}
                )
                if (

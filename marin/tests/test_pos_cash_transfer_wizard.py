...
class TestPOSCashTransferWizard(TestPoSCommon):
    ...
    def setUp(self):
        ...
        self.cash_2 = self.env["account.journal"].create(
            {"name": "Test Cash", "code": "TCSH", "type": "cash"}
        )
        self.user_partner = self.env["res.partner"].search(
            [("user_ids", "=", self.env.user.id)], limit=1
        )
    ...
    def test_cash_transfer(self):
        ...
        wizard = self.env["pos.cash.transfer.wizard"].create(
            {
                ...
                "destination_account_id": cash_transfer_wizard["context"][
        ...
        self.assertFalse(
            self.env["mail.activity"].search(
                [("activity_type_id", "=", cash_transfer_activity_id)]
            )
        )

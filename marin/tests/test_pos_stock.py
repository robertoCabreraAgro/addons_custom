...
class TestPoSStock(TestPoSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env.user.group_ids = [
            (4, cls.env.ref("marin.group_stock_inventory_adjustment").id)
        ]
    ...
    def test_01_pos_stock_availability(self):
        self.LotObj.get_available_lots_for_pos(
            self.product_a.id, self.company.id, self.config.id
        )
    ...
    def test_02_get_removal_strategies(self):
        strat = self.quant_p_a._get_removal_strategy_domain_order(
            [], "fifo + priority", 1
        )[1]
        ...
        _sortkey, reverse = self.quant_p_a._get_removal_strategy_sort_key(
            "fifo + priority"
        )
    ...
    def test_03_error_validate_quant(self):
        self.env.user.group_ids = [
            (3, self.env.ref("marin.group_stock_inventory_adjustment").id)
        ]
        ...
        with self.assertRaisesRegex(
            UserError, "Only a Inventory manager can validate an inventory adjustment."
        ):
            quant.action_apply_inventory()
        self.env.user.group_ids = [
            (4, self.env.ref("marin.group_stock_inventory_adjustment").id)
        ]
        quant.action_apply_inventory()

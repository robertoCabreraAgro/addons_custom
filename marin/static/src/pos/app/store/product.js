...
patch(Product.prototype, {
    async getAddProductOptions(code) {
        ...
        if (this.combo_ids.length) {
            const {confirmed, payload} = await this.env.services.popup.add(ComboConfiguratorPopup, {
                product: this,
                keepBehind: true,
            });
            if (!confirmed) {
        ...
        if (this.to_weight && this.pos.config.iface_electronic_scale) {
            ...
            if (this.isScaleAvailable) {
                const product = this;
                const {confirmed, payload} = await this.env.services.pos.showTempScreen("ScaleScreen", {
                    product,
                });
                if (confirmed) {

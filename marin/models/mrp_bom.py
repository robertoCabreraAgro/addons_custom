...
class MrpBom(models.Model):
    ...
    def _check_bom_cycle(self):
        boms_to_validate = self.filtered(
            lambda bom: bom.production_type != "reformulated"
        )
        if boms_to_validate:
        ...
        def _check_cycle(components, finished_products):
            ...
            if components_to_find:
                products_to_find = self.env["product.product"].union(
                    *components_to_find
                )
            ...
            if has_variant_attrs:
                ...
                for finished in finished_products:
                    components = bom.bom_line_ids.filtered(
                        lambda line: not line._skip_bom_line(finished)
                    ).product_id

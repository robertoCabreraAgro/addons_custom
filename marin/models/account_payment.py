        ...
        for payment in self:
            try:
                ...
                payment_lines = payment.move_id.line_ids.filtered(
                    lambda l: l.account_id.account_type == account_type and l.amount_residual != 0
                )
                
                if not payment_lines:
                    errors.append(_("Payment %s has no lines to reconcile") % payment.name)
                    continue
                ...
                for payment_line in payment_lines:
                    ...
                    invoice_lines = self.env['account.move.line'].search(domain, order='date asc, id asc')
                    
                    if not invoice_lines:
                        continue
                    
                    # Try combinations of invoices to match the payment amount
                    invoice_amounts = [(line, abs(line.amount_residual)) for line in invoice_lines]
                    ...
                    for invoice_line, invoice_amount in invoice_amounts:
                        if abs(invoice_amount - target_amount) < 0.01:
                            ...
                            invoice_line.move_id.message_post(
                                body=Markup(_('Automatic reconciliation applied: <strong>Multi-Invoice (Single)</strong> - Payment matched exactly to this invoice')),
                    ...
                    for r in range(2, max_combination_size + 1):
                        ...
                        for combo in combinations(invoice_amounts, r):
                            ...
                            if abs(combo_sum - target_amount) < 0.01:
                                ...
                                lines_to_reconcile = payment_line + self.env['account.move.line'].browse([l.id for l in combo_lines])
                                lines_to_reconcile.reconcile()
                                # Add post-message to each reconciled invoice
                                for combo_line in combo_lines:
                                    combo_line.move_id.message_post(
                                        body=Markup(_('Automatic reconciliation applied: <strong>Multi-Invoice</strong> - Combined with %d other invoices for payment matching') % (len(combo_lines) - 1)),
                                        message_type='notification'
                                    )
                                reconciled_count += 1
                                found_combination = True
                ...
                errors.append(_("Error reconciling payment %s: %s") % (payment.name, str(e)))
        
        # Prepare result message
        if reconciled_count > 0:
            message = _("%d payment(s) reconciled successfully with multiple invoices.") % reconciled_count
        else:

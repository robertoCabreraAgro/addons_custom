...
def fix_orphaned_cron_references(cr):
    ...
    _logger.info(
        "Deleted %s cron jobs with orphaned references", deleted_orphaned_crons
    )
    ...
    _logger.info(
        "Pre-migration completed successfully - eliminated %s cron jobs, %s server actions, and %s model data entries",
        deleted_crons + deleted_orphaned_crons,
        deleted_servers,
        deleted_model_data,
    )

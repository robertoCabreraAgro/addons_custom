...
def _cleanup_approval_request_vehicle_refs(cr):
    ...
    if not cr.fetchone()[0]:
        _logger.info(
            "approval_request.vehicle_id column does not exist, skipping cleanup"
        )
        return
    ...
    if invalid_count > 0:
        _logger.info(
            f"Found {invalid_count} approval_request records with invalid vehicle_id references"
        )
        ...
        _logger.info(
            f"Cleaned {cleaned_count} invalid vehicle_id references in approval_request"
        )
    ...
    for table_name in tables_to_check:
        try:
            ...
            if cr.fetchone()[0]:
                ...
                if invalid_count > 0:
                    _logger.info(
                        f"Found {invalid_count} invalid vehicle_id references in {table_name}"
                    )
                    cr.execute(
                        f"""
                        UPDATE {table_name}
                    ...
                    cleaned_count = cr.rowcount
                    _logger.info(
                        f"Cleaned {cleaned_count} invalid vehicle_id references in {table_name}"
                    )

...
def migrate(cr, version):
    ...
    """
    _logger.info("Starting Marin approval system migration from fleet to product_asset...")
...
def _migrate_approval_category_types(cr, env):
    ...
    try:
        ...
        _logger.info(f"Found {old_count} approval_category records with old approval_type")
        ...
        if old_count > 0:
            ...
            _logger.info(f"Updated {updated_count} approval_category records with new approval_type")
        else:
...
def _migrate_approval_request_vehicles(cr, env):
    ...
    for request_id, old_vehicle_id in requests_data:
        try:
            ...
            if result:
                ...
                migrated_count += 1
                _logger.debug(f"Request {request_id}: fleet.vehicle({old_vehicle_id}) -> stock.lot({new_asset_id})")
            else:
                _logger.warning(f"No matching stock.lot found for request {request_id} with vehicle {old_vehicle_id}")
                failed_count += 1
    ...
    _logger.info(f"Approval request vehicles migration: {migrated_count} migrated, {failed_count} failed")
...
def _migrate_approval_request_logs(cr, env):
    ...
    _logger.info(f"Found {migrated_logs_count} product.asset.log records linked to approval_request")
...
def _validate_migration(cr, env):
    ...
    if old_categories > 0 or old_requests > 0:
        _logger.warning(f"Found {old_categories + old_requests} records with old fleet references")
    else:

...
class CrmTeamMember(models.Model):
    ...
    def _constrains_membership(self):
        ...
        if duplicates:
            raise exceptions.ValidationError(_(
                ...
                duplicates=", ".join(
                    "%s (%s)" % (m.user_id.name, m.work_team_id.name) for m in duplicates
                )
            ))
    ...
    def _compute_user_in_teams_ids(self):
        ...
        if all(m.is_membership_multi for m in self):
            ...
            member_user_ids = self.env['work.team.member'].search([('id', 'not in', self.ids)]).user_id
        else:
        ...
        for member in self:
            if member_user_ids:
                ...
                member.user_in_teams_ids = self.env['work.team'].browse(self.env.context['default_work_team_id']).member_ids
            else:
    ...
    def _compute_member_warning(self):
        ...
        if all(m.is_membership_multi for m in self):
            ...
            existing = self.env['work.team.member'].search([('user_id', 'in', active.user_id.ids)])
    ...
    def create(self, values_list):
        ...
        (tracked fields).
        """
        is_membership_multi = self.env['ir.config_parameter'].sudo().get_param('work_team.membership_multi', False)
    ...
    def write(self, values):
        ...
        if not is_membership_multi and values.get('active'):
            self._synchronize_memberships([
                dict(user_id=membership.user_id.id, work_team_id=membership.work_team_id.id)
                for membership in self
            ])
    ...
    def _synchronize_memberships(self, user_team_ids):
        ...
        user_memberships = dict.fromkeys(existing.user_id.ids, self.env['work.team.member'])
        for membership in existing:
        ...
        for values in user_team_ids:
            existing_to_archive += user_memberships.get(values['user_id'], self.env['work.team.member']).filtered(
                lambda m: m.work_team_id.id != values['work_team_id']
            )

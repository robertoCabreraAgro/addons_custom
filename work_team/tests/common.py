...
class SalesTeamCommon(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.group_sale_salesman = cls.env.ref('work_team.group_sale_salesman')
        cls.group_sale_manager = cls.env.ref('work_team.group_sale_manager')
        ...
        cls.sale_team = cls.env['work.team'].create({
            'name': 'Test Work Team',
        })
        # Disable other teams (demo data/existing data)
        cls.env['work.team'].search([
            ('id', '!=', cls.sale_team.id),
        ]).action_archive()
...
class TestSalesCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env['ir.config_parameter'].set_param('work_team.membership_multi', False)
        ...
        cls.user_sales_manager = mail_new_test_user(
            ...
            groups='work_team.group_sale_manager,base.group_partner_manager',
        )
        cls.user_sales_leads = mail_new_test_user(
            ...
            company_id=cls.company_main.id,
            notification_type='inbox',
            groups='work_team.group_sale_salesman_all_leads,base.group_partner_manager',
        )
        cls.user_sales_salesman = mail_new_test_user(
            ...
            company_id=cls.company_main.id,
            notification_type='inbox',
            groups='work_team.group_sale_salesman',
        )

        cls.env['work.team'].search([]).write({'sequence': 9999})
        cls.sales_team_1 = cls.env['work.team'].create({
            'name': 'Test Work Team',
            'sequence': 5,
            'company_id': False,
            'user_id': cls.user_sales_manager.id,
        })
        cls.sales_team_1_m1 = cls.env['work.team.member'].create({
            'user_id': cls.user_sales_leads.id,
            'work_team_id': cls.sales_team_1.id,
        })
        cls.sales_team_1_m2 = cls.env['work.team.member'].create({
...
class TestSalesMC(TestSalesCommon):
    """ Multi Company / Multi Work Team environment """
    ...
    def setUpClass(cls):
        ...
        cls.team_mc = cls.env['work.team'].create({

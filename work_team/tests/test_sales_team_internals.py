...
from odoo.addons.work_team.tests.common import TestSalesMC
...
class TestCornerCases(TransactionCase):
    ...
    def setUp(self):
        ...
        self.sales_team_1 = self.env['work.team'].create({
            'name': 'Test Work Team',
            'sequence': 5,
            'company_id': False,
    ...
    def test_unicity(self):
        ...
        sales_team_1_m1 = self.env['work.team.member'].create({
            'user_id': self.user_sales_leads.id,
            'work_team_id': self.sales_team_1.id,
        })

        sales_team_1_m1.write({'active': False})
        sales_team_1_m1.flush_recordset()

        sales_team_1_m2 = self.env['work.team.member'].create({
            'user_id': self.user_sales_leads.id,
            'work_team_id': self.sales_team_1.id,
        })

        found = self.env['work.team.member'].search([
            ('user_id', '=', self.user_sales_leads.id),
            ('work_team_id', '=', self.sales_team_1.id),
        ])
    ...
    def test_unicity_multicreate(self):
        ...
        with self.assertRaises(exceptions.UserError), mute_logger('odoo.sql_db'):
            self.env['work.team.member'].create([
                {'user_id': self.user_sales_leads.id, 'work_team_id': self.sales_team_1.id},
                {'user_id': self.user_sales_leads.id, 'work_team_id': self.sales_team_1.id}
            ])
...
class TestSecurity(TestSalesMC):
    ...
    def test_team_access(self):
        ...
        work_team.read(['name'])
        for member in work_team.member_ids:
            member.read(['name'])

        with self.assertRaises(exceptions.AccessError):
            work_team.write({'name': 'Trolling'})

        for membership in work_team.work_team_member_ids:
            membership.read(['name'])
            with self.assertRaises(exceptions.AccessError):
                membership.write({'active': False})

        with self.assertRaises(exceptions.AccessError):
            work_team.write({'member_ids': [(5, 0)]})

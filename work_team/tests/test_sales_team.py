...
from odoo.addons.work_team.tests.common import SalesTeamCommon, TestSalesCommon, TestSalesMC
...
class TestDefaultTeam(TestSalesCommon):
    ...
    def setUpClass(cls):
        ...
        cls.team_c2 = cls.env['work.team'].create({
            'name': 'C2 Team1',
            'sequence': 1,
            'company_id': cls.company_2.id,
            'user_id': False,
        })
        cls.team_sequence = cls.env['work.team'].create({
            'company_id': False,
        ...
        cls.team_responsible = cls.env['work.team'].create({
    ...
    def test_default_team_fallback(self):
        ...
        self.assertFalse(self.env['work.team.member'].search([('user_id', '=', self.user_sales_leads.id)]))

        # default is better sequence matching company criterion
        with self.with_user('user_sales_leads'):
            team = self.env['work.team']._get_default_team_id()
    ...
    def test_default_team_wcontext(self):
        ...
        with self.with_user('user_sales_leads'):
            ...
            team = self.env['work.team'].with_context(
...
class TestMultiCompany(TestSalesMC):
    ...
    def test_team_members(self):
        ...
        team_c2 = self.env['work.team'].browse(self.team_c2.id)
    ...
    def test_team_memberships(self):
        ...
        # can add someone from same company
        self.env.user.write({'company_id': self.company_2.id})
        team_c2.write({'work_team_member_ids': [(0, 0, {'user_id': self.env.user.id})]})
        self.assertEqual(team_c2.member_ids, self.env.user)

        # cannot add someone from another company
        with self.assertRaises(exceptions.UserError):
            team_c2.write({'work_team_member_ids': [(0, 0, {'user_id': self.user_sales_salesman.id})]})
...
class TestAccessRights(SalesTeamCommon):
    ...
    def test_access_sales_manager(self):
        ...
        # Manager can create a Work Team
        india_channel = self.env['work.team'].with_context(tracking_disable=True).create({
            'name': 'India',
        })
        self.assertIn(
            india_channel.id, self.env['work.team'].search([]).ids,
            'Sales manager should be able to create a Work Team')

        # Manager can edit a Work Team
        india_channel.write({'name': 'new_india'})
        self.assertEqual(
            india_channel.name, 'new_india',
            'Sales manager should be able to edit a Work Team')

        # Manager can delete a Work Team
        india_channel.unlink()
        self.assertNotIn(
            india_channel.id, self.env['work.team'].search([]).ids,
            'Sales manager should be able to delete a Work Team')

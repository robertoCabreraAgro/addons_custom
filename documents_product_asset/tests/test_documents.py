...
TEXT = base64.b64encode(bytes("documents_product_asset", "utf-8"))
...
class TestCaseDocumentsBridgeFleet(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.fleet_folder = cls.env.ref("documents_product_asset.document_folder_fleet")
        company = cls.env.user.company_id
        company.documents_settings_fleet = True
        company.documents_folder_fleet_id = cls.fleet_folder
        cls.manager_1 = new_test_user(
            cls.env,
            "test fleet manager",
            groups="documents.group_documents_user, fleet.fleet_group_manager",
        )
        cls.manager_2 = new_test_user(
            cls.env, "test fleet manager 2", groups="fleet.fleet_group_manager"
        )
    ...
    def test_fleet_attachment(self):
        ...
        self.env["documents.access"].create(
            [
                {
                    "document_id": self.fleet_folder.id,
                    "partner_id": self.manager_1.partner_id.id,
                    "role": "edit",
                },
                {
                    "document_id": self.fleet_folder.id,
                    "partner_id": self.manager_2.partner_id.id,
                    "role": "edit",
                },
            ]
        )
        ...
        self.assertEqual(
            document.res_id,
            self.fleet_vehicle.id,
            "fleet record linked to the document ",
        )
        self.assertEqual(
            document.owner_id,
            self.manager_1,
            "default document owner is the current user",
        )
        self.assertEqual(
            document.res_model,
            self.fleet_vehicle._name,
            "fleet model linked to the document",
        )
    ...
    def test_disable_fleet_centralize_option(self):
        ...
        company.documents_settings_fleet = False
    ...
    def test_company_reset_documents_folder_id(self):
        ...
        self.assertEqual(new_company.documents_folder_fleet_id, self.fleet_folder)
        ...
        self.env["res.company"].sudo().search(
            [("id", "not in", test_companies.ids)]
        ).documents_folder_fleet_id = False

        fleet_folder_2 = self.env["documents.document"].create(
            {"name": "Fleet Folder 2", "type": "folder"}
        )

        self.env.company.documents_settings_fleet = False
        self.assertEqual(
            test_companies.with_context(
                allowed_company_ids=test_companies.ids
            ).documents_folder_fleet_id,
            self.fleet_folder,
        ...
        new_company.documents_folder_fleet_id = fleet_folder_2
        self.fleet_folder.action_archive()

        new_company.documents_settings_fleet = False
        self.assertEqual(
            new_company.documents_folder_fleet_id,
            fleet_folder_2,
            "The folder should still be linked with disabled bridge",
        )

        new_company.documents_settings_fleet = True
        self.assertEqual(new_company.documents_folder_fleet_id, fleet_folder_2)
        new_company.documents_settings_fleet = False
        fleet_folder_2.unlink()
        new_company.documents_settings_fleet = True
        self.assertFalse(
            new_company.documents_folder_fleet_id,
            "There should be no value as the default is archived",
        )
        new_company.documents_settings_fleet = False

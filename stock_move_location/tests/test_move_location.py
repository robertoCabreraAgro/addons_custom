...
class TestMoveLocation(TestsCommon):
    ...
    def test_move_location_wizard_amount(self):
        ...
        with self.assertRaises(ValidationError):
            wizard.line_ids[0].move_quantity += 1
    ...
    def test_quant_transfer(self):
        ...
        wizard = self.wizard_obj.with_context(
            ...
            location_origin_readonly=True,
        ).create(
            {
                "location_origin_id": quants[:1].location_id.id,
                "location_destination_id": self.internal_loc_2.id,
            }
        )
        ...
        wizard.onchange_origin_location()
        self.assertEqual(len(lines), 3)
        wizard.location_destination_id = self.internal_loc_1
        wizard._onchange_location_destination_id()
        self.assertEqual(lines.mapped("location_destination_id"), self.internal_loc_1)
        wizard.location_origin_id = self.internal_loc_2
        wizard._onchange_location_destination_id()
        self.assertEqual(len(lines), 3)

    def test_readonly_location_computation(self):
        """Test that location_origin_readonly and location_destination_readonly
        are computed correctly."""
        wizard = self._create_wizard(self.internal_loc_1, self.internal_loc_2)
        # locations are editable.
        self.assertFalse(wizard.location_origin_readonly)
        self.assertFalse(wizard.location_destination_readonly)
        # Disable edit mode:
        wizard.edit_locations = False
        self.assertTrue(wizard.location_origin_readonly)
        self.assertTrue(wizard.location_destination_readonly)
    ...
    def test_wizard_with_putaway_strategy(self):
        ...
        wizard.onchange_origin_location()
        putaway_line = wizard.line_ids.filtered(
            lambda p: p.product_id == self.product_no_lots
        )[0]
        self.assertEqual(
            putaway_line.location_destination_id, self.internal_loc_2_shelf
        )

    ...
    for category_type, (old_xmlid, new_xmlid) in category_mappings.items():
        try:
            ...
            if old_result and new_result:
                ...
                _logger.info(
                    f"Old {category_type} category not found - might already be migrated"
                )
            elif not new_result:
                _logger.warning(
                    f"New {category_type} category not found in product_asset module"
                )
...
def _migrate_log_products(cr, env):
    ...
    product_mappings = {
        "fuel_debit": (
            "marin.product_product_fuel_debit",
            "product_asset.product_product_fuel_debit",
        ),
        "fuel_credit": (
            "marin.product_product_fuel_credit",
            "product_asset.product_product_fuel_credit",
        ),
        "highway_debit": (
    ...
    for product_type, (old_xmlid, new_xmlid) in product_mappings.items():
        try:
            ...
            if old_result and new_result:
                ...
                _logger.info(
                    f"Old {product_type} product not found - might already be migrated"
                )
            elif not new_result:
                _logger.warning(
                    f"New {product_type} product not found in product_asset module"
                )

...
def post_init_hook(env):
    ...
    try:
        ...
        if not _check_fleet_module_availability(env):
            _logger.info(
                "Fleet module not found or no data to migrate. Skipping migration."
            )
            return
...
def _reorganize_partner_ids(env):
    ...
    for (partner_id,) in partners_data:
        try:
            # Update partner ID
            cr.execute(
                "UPDATE res_partner SET id = %s WHERE id = %s", (start_id, partner_id)
            )
...
def _migrate_fleet_to_product_asset(env):
    ...
    for model_id, model_name, vehicle_ids in models_data:
        try:
            ...
            _logger.debug(
                f"Created product template: {model_name} (ID: {product_template.id})"
            )

            # Create stock.lot for each vehicle of this model
            for vehicle_id in vehicle_ids:
                migrated_count = _migrate_single_vehicle(
                    env, vehicle_id, product_template
                )
    ...
    _logger.info(
        f"Migration completed: {created_templates} templates, {migrated_vehicles} vehicles"
    )
...
def _migrate_single_vehicle(env, vehicle_id, product_template):
    ...
    optional_columns = [
        ...
        "acquisition_date",
        "original_value",
        "fuel_card_openning_balance",
        "fuel_card_budget",
        "highway_pass_openning_balance",
        "highway_pass_budget",
    ]
...
def _migrate_vehicle_logs(env, vehicle_id, lot_id):
    ...
    if cr.fetchone()[0] == 0:
        _logger.debug(
            f"No fleet_vehicle_log table found, skipping log migration for vehicle {vehicle_id}"
        )
        return
    ...
    # Get logs data
    query = (
        f"SELECT {', '.join(all_columns)} FROM fleet_vehicle_log WHERE vehicle_id = %s"
    )
    ...
    for log_data in logs_data:
        ...
        try:
            log_vals = {
                ...
                "company_id": env["stock.lot"]
                .browse(lot_id)
                .company_id.id,  # Use same company as the asset
            }
...
def _validate_migration_results(env):
    ...
    if migrated_assets < original_vehicles:
        _logger.warning(
            f"Migration incomplete: {original_vehicles - migrated_assets} vehicles not migrated"
        )
    else:

...
@tagged("payroll_input_batch")
class TestHrPayrollInputBatch(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_001_inputs(self):
        ...
        # Set absences to be included in the current payroll and avoid validation error when running
        # compute_sheet method
        # https://github.com/odoo/enterprise/blob/18.0/hr_payroll_holidays/models/hr_payslip.py#L24.
        self.env["hr.leave"].search(
            [("employee_id", "in", payroll.employee_id.ids), ("state", "!=", "refuse")]
        ).payslip_state = "done"
        # Prepare batch

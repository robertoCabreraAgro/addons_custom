...
def where_calc(model, domain, active_test=True, alias=None):
    ...
    if model._active_name and active_test and model._context.get("active_test", True):
        if not any(
            item[0] == model._active_name or
            (isinstance(item[0], str) and '.' in item[0] and item[0].split('.', 1)[0] == model._active_name)
            for item in domain if isinstance(item, (list, tuple)) and len(item) > 0
        ):
            domain = [(model._active_name, "=", 1)] + domain

    # Asegurarnos de que 'alias' es una cadena
    if alias is None:
        alias = model._table  # Usar el nombre de la tabla como alias si no se proporciona uno

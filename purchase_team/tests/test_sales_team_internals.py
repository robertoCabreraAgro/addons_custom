...
from odoo.addons.purchase_team.tests.common import TestSalesMC
...
class TestCornerCases(TransactionCase):
    def setUp(self):
        ...
        self.sales_team_1 = self.env["srm.team"].create(
            {
    ...
    def test_unicity(self):
        ...
        sales_team_1_m1 = self.env["srm.team.member"].create(
            {
                "user_id": self.user_sales_leads.id,
                "srm_team_id": self.sales_team_1.id,
            }
        )
        ...
        sales_team_1_m2 = self.env["srm.team.member"].create(
            {
                "user_id": self.user_sales_leads.id,
                "srm_team_id": self.sales_team_1.id,
            }
        ...
        found = self.env["srm.team.member"].search(
            [
                ("user_id", "=", self.user_sales_leads.id),
                ("srm_team_id", "=", self.sales_team_1.id),
            ]
    ...
    def test_unicity_multicreate(self):
        ...
        with self.assertRaises(exceptions.UserError), mute_logger("odoo.sql_db"):
            self.env["srm.team.member"].create(
                [
                    {"user_id": self.user_sales_leads.id, "srm_team_id": self.sales_team_1.id},
                    {"user_id": self.user_sales_leads.id, "srm_team_id": self.sales_team_1.id},
                ]
            )
...
class TestSecurity(TestSalesMC):
    ...
    def test_team_access(self):
        self.sales_team_1.with_user(self.env.user)

        self.sales_team_1.read(["name"])
        for member in self.sales_team_1.member_ids:
            member.read(["name"])

        # with self.assertRaises(exceptions.AccessError):
        #     self.sales_team_1.write({"name": "Trolling"})

        for membership in self.sales_team_1.srm_team_member_ids:
            membership.read(["name"])
            # with self.assertRaises(exceptions.AccessError):
            #     membership.write({"active": False})

        # with self.assertRaises(exceptions.AccessError):
        #     self.sales_team_1.write({"member_ids": [(5, 0)]})

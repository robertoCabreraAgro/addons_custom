...
from odoo.addons.purchase_team.tests.common import (
    PurchasesTeamCommon,
    TestSalesCommon,
    TestSalesMC,
)
...
class TestDefaultTeam(TestSalesCommon):
    ...
    def setUpClass(cls):
        ...
        # buyer---------------sales_team_1---C2Team1---LowSequ---Team3
        # admin------------------M-------------- --------- ---------
        ...
        cls.team_sequence = cls.env["srm.team"].create(
            {
                "company_id": False,
        ...
        cls.team_responsible = cls.env["srm.team"].create(
            {
    ...
    def test_default_team_fallback(self):
        ...
        self.assertFalse(self.env["srm.team.member"].search([("user_id", "=", self.user_sales_leads.id)]))

        # default is better sequence matching company criterion
        with self.with_user("user_sales_leads"):
            team = self.env["srm.team"]._get_default_team_id()
    ...
    def test_default_team_wcontext(self):
        ...
        with self.with_user("user_sales_leads"):
            ...
            team = self.env["srm.team"].with_context(default_team_id=self.sales_team_1.id)._get_default_team_id()
            self.assertEqual(
                team, self.sales_team_1, "PurchasesTeam: default takes over ordering when member / responsible"
            )
        ...
        with self.with_user("user_sales_leads"):
            ...
            team = self.env["srm.team"].with_context(default_team_id=self.sales_team_1.id)._get_default_team_id()
            self.assertEqual(
                team, self.sales_team_1, "PurchasesTeam: default taken into account when no member / responsible"
            )
...
class TestMultiCompany(TestSalesMC):
    ...
    def test_team_members(self):
        ...
        team_c2 = self.env["srm.team"].browse(self.team_c2.id)
        ...
        with self.assertRaises(exceptions.UserError):
            team_c2.write({"member_ids": [(4, self.user_sales_buyer.id)]})

        # reset members, change company
        team_c2.write({"member_ids": [(5, 0)], "company_id": self.company_main.id})
        self.assertEqual(team_c2.member_ids, self.env["res.users"])
        team_c2.write({"member_ids": [(4, self.user_sales_buyer.id)]})
        self.assertEqual(team_c2.member_ids, self.user_sales_buyer)
    ...
    def test_team_memberships(self):
        ...
        # can add someone from same company
        self.env.user.write({"company_id": self.company_2.id})
        team_c2.write({"srm_team_member_ids": [(0, 0, {"user_id": self.env.user.id})]})
        self.assertEqual(team_c2.member_ids, self.env.user)

        # cannot add someone from another company
        with self.assertRaises(exceptions.UserError):
            team_c2.write({"srm_team_member_ids": [(0, 0, {"user_id": self.user_sales_buyer.id})]})
...
class TestAccessRights(PurchasesTeamCommon):
    @users("purchasemanager")

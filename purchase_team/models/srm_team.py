...
class SrmTeam(models.Model):
    ...
    def _get_default_team_id(self, user_id=False, domain=False):
        ...
        :param user_id: Purchase representative to target, fallback on env.uid;
        ...
        default_team = (
            self.env["srm.team"].browse(self.env.context["default_team_id"])
            if self.env.context.get("default_team_id")
            else self.env["srm.team"]
        )
        ...
        team = self.env["srm.team"]
        teams = self.env["srm.team"].search(
        ...
        if not team:
            teams = self.env["srm.team"].search([("company_id", "in", valid_cids)])
    ...
    user_id = fields.Many2one("res.users", "Team Leader", check_company=True)
    active = fields.Boolean(default=True)
    sequence = fields.Integer("Sequence", default=10)
    color = fields.Integer("Color Index", help="The color of the channel")
    srm_team_member_ids = fields.One2many(
        "srm.team.member",
        "srm_team_id",
        "Purchases Team Members",
        context={"active_test": True},
        help="Add members to automatically assign their documents to this purchases team.",
    )
    srm_team_member_all_ids = fields.One2many(
        "srm.team.member",
        "srm_team_id",
        "Purchases Team Members (incl. inactive)",
    ...
    member_company_ids = fields.Many2many(
        "res.company", compute="_compute_member_company_ids", help="UX: Limit to team company or all if no company"
    )
    favorite_user_ids = fields.Many2many(
        "res.users",
        "srm_team_res_users_favorite_rel",
        "team_id",
        "user_id",
        "Favorite Members",
    ...
    dashboard_button_name = fields.Char("Dashboard Button", compute="_compute_dashboard_button_name")
    dashboard_graph_data = fields.Text("_compute_dashboard_graph")
    ...
    def _compute_member_ids(self):
        for team in self:
            team.member_ids = team.srm_team_member_ids.user_id
    ...
    def _inverse_member_ids(self):
        for team in self:
            ...
            memberships = team.srm_team_member_ids
            users_current = team.member_ids
            users_new = users_current - memberships.user_id

            # add missing memberships
            self.env["srm.team.member"].create([{"srm_team_id": team.id, "user_id": user.id} for user in users_new])
    ...
    def _compute_member_warning(self):
        ...
        for team in self:
            ...
            other_memberships = self.env["srm.team.member"].search(
                [
                    ("srm_team_id", "!=", team.id if team.ids else False),  # handle NewID
            ...
            if other_memberships and len(other_memberships) == 1:
                member_warning = _(
                    ...
                    team_name=other_memberships.srm_team_id.name,
                )
                ...
                member_warning = _(
                    ...
                    team_names=", ".join(other_memberships.mapped("srm_team_id.name")),
                )
            if member_warning:
                team.member_warning = (
                    member_warning
                    + " "
                    + _(
                        "To add a Purchase representative into multiple Teams, "
                        "activate the Multi-Team option in settings."
                    )
                )

    def _search_member_ids(self, operator, value):
        return [("srm_team_member_ids.user_id", operator, value)]
    ...
    def _compute_dashboard_button_name(self):
        """Sets the adequate dashboard button name depending on the Purchases Team's options"""
        for team in self:
    ...
    def create(self, vals_list):
        teams = super(SrmTeam, self.with_context(mail_create_nosubscribe=True)).create(vals_list)
    ...
    def write(self, values):
        ...
        if values.get("company_id"):
            self.srm_team_member_ids._check_company(fnames=["srm_team_id"])
    ...
    def _unlink_except_default(self):
        default_teams = [
            self.env.ref("purchase_team.team_purchases_department"),
        ]
        for team in self:
    ...
    def action_primary_channel_button(self):
        ...
        depending on the Purchases Team's options."""
        return False
    ...
    def _graph_get_model(self):
        ...
        raise UserError(_("Undefined graph model for Purchases Team: %s", self.name))
    ...
    def _graph_get_table(self, graph_model):
        return graph_model._table
    ...
    def _graph_data(self, start_date, end_date):
        ...
        graph_model = self.env[dashboard_graph_model]
        graph_table = self._graph_get_table(graph_model)
        extra_conditions = self._extra_sql_conditions()
        where_query = graph_model._where_calc([])
        graph_model._apply_ir_rules(where_query, "read")

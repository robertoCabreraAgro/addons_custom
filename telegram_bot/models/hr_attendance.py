...
class HrAttendance(models.Model):
    ...
    def _cron_auto_checkout_at_day_end(self):
        ...
        open_attendances = self.search(
            [("check_in", ">=", yesterday_start_utc), ("check_in", "<", today_start_utc), ("check_out", "=", False)]
        )

        for att in open_attendances:
            employee = att.employee_id
            if not employee.resource_calendar_id:
                _logger.warning("Skipping auto-checkout for attendance %s: Employee or calendar not found.", att.id)
                continue
            ...
            _logger.info("Automatically checked out attendance %s for employee %s.", att.id, employee.name)

...
class StockLotRule(models.Model):
    ...
    name = fields.Char(
        string="Name", required=True, tracking=True, help="Name of the lot rule schema"
    )
    ...
    def _generate_lot_name(self, partial_name, product, ref_value=None):
        ...
        result = self._verify_lot_name_format(partial_name, product.id)
        if result["valid"]:
            return partial_name
        ...
        try:
            ...
            return self._sanitize_lot_name(suggested_name, product.id)
        except Exception:
            # Ultimate fallback
            return partial_name

    def _sanitize_lot_name(self, lot_name, product_id):
        """
        ...
        """
        validation = self._verify_lot_name_format(lot_name, product_id)

...
def post_init_hook(env):
    ...
    try:
        ...
        if not _check_account_fleet_module_availability(env):
            _logger.info(
                "Account Fleet module not found or no data to migrate. Skipping migration."
            )
            return
        ...
        _logger.info(
            "=== Account Product Asset post-installation hook completed successfully ==="
        )
...
def _migrate_account_fleet_to_product_asset(env):
    ...
    _logger.info(
        f"Migration completed: {migrated_move_lines} move lines, {migrated_moves} moves"
    )
...
def _migrate_move_line_vehicle_references(env):
    ...
    _logger.info(
        f"Database contains: {fleet_count} fleet_vehicles, {stock_count} stock_lot vehicles"
    )
    ...
    cr.execute(
        "SELECT id, name, license_plate FROM stock_lot WHERE asset_type = 'vehicle' LIMIT 3"
    )
    ...
    try:
        ...
        if potential_matches == 0:
            if fleet_count > 0 and stock_count == 0:
                raise UserError(
                    "No stock.lot vehicles found but fleet_vehicle records exist. "
                    "Please ensure product_asset module is installed and migration completed first."
                )
            elif fleet_count > 0 and stock_count > 0:
                _logger.warning(
                    f"Data mismatch: {fleet_count} fleet vehicles but {stock_count} stock vehicles - "
    ...
    for move_line_id, old_vehicle_id in move_lines_data:
        try:
            ...
            if result:
                ...
                migrated_count += 1
                _logger.debug(
                    f"Move line {move_line_id}: fleet.vehicle({old_vehicle_id}) -> stock.lot({new_asset_id})"
                )
            else:
                # No matching stock.lot found, leave asset_id as NULL
                _logger.debug(
                    f"No matching stock.lot found for move line {move_line_id} with vehicle {old_vehicle_id}"
                )
                nullified_count += 1
    ...
    _logger.info(
        f"Move lines migration: {migrated_count} migrated, {nullified_count} nullified, {failed_count} failed"
    )

    # Note: We don't recreate the vehicle_id constraint since we're using asset_id field instead
    _logger.info(
        "Migration completed - data moved from old_vehicle_id to asset_id field"
    )
    ...
    _logger.info(
        f"Migration success rate: {success_rate:.2%} ({migrated_count}/{total_processed})"
    )

    if (
        total_processed > 0 and success_rate < 0.5
    ):  # Less than 50% successfully migrated
        raise UserError(
            f"Migration failed: Only {success_rate:.1%} of records successfully migrated. "
            f"Expected better mapping between fleet_vehicle and stock_lot. "
            f"Check data consistency before proceeding."
        )

    if (
        nullified_count > migrated_count and total_processed > 100
    ):  # More nullified than migrated (only for large datasets)
        _logger.warning(
            f"Warning: {nullified_count} records were set to NULL vs {migrated_count} migrated"
        )
...
def _migrate_move_vehicle_references(env):
    ...
    for move_id, old_vehicle_id in moves_data:
        try:
            ...
            if result:
                ...
                migrated_count += 1
                _logger.debug(
                    f"Move {move_id}: fleet.vehicle({old_vehicle_id}) -> stock.lot({new_asset_id})"
                )
            else:
                # Vehicle not migrated, set to NULL to avoid issues
                _logger.warning(
                    f"No matching stock.lot found for move {move_id} with vehicle {old_vehicle_id}, setting to NULL"
                )
                cr.execute(
                    """
                    UPDATE account_move 
    ...
    _logger.info(
        f"Moves migration: {migrated_count} migrated, {nullified_count} nullified, {failed_count} failed"
    )
    return migrated_count
...
def _validate_migration_results(env):
    ...
    if orphaned_move_lines > 0 or orphaned_moves > 0:
        _logger.warning(
            f"Migration incomplete: {orphaned_move_lines + orphaned_moves} records with invalid references"
        )
    else:
